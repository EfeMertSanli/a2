...
public class Main {
    ...
    private static final RecommendedCommandParser recommendParser = new RecommendedCommandParser();
    ...
    private static void handleRecommendCommand(String commandStr) {
        try {
            ...
            RecommendedCommandParser.RecommendTerm term = recommendParser.parse(commandStr);
            ...
            int productId = 0;
            if (term instanceof RecommendedCommandParser.FinalTerm) {
                productId = ((RecommendedCommandParser.FinalTerm) term).getProductId();
            } else {
     ...
     */
    private static int findFirstProductId(RecommendedCommandParser.RecommendTerm term) {
        if (term instanceof RecommendedCommandParser.FinalTerm) {
            return ((RecommendedCommandParser.FinalTerm) term).getProductId();
        } else if (term instanceof RecommendedCommandParser.IntersectionTerm) {
            RecommendedCommandParser.IntersectionTerm intersectionTerm = (RecommendedCommandParser.IntersectionTerm) term;
            try {
                return findFirstProductId(intersectionTerm.getLeft());
            } catch (IllegalArgumentException e) {
                return findFirstProductId(intersectionTerm.getRight());
            }
        } else if (term instanceof RecommendedCommandParser.UnionTerm) {
            RecommendedCommandParser.UnionTerm unionTerm = (RecommendedCommandParser.UnionTerm) term;
            try {
